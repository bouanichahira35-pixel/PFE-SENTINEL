...
// Ce fichier sert de script de maintenance, seed, test ou migration pour test-critical-flow.
...
async function run() {
  ...
  // Ensure known test users/passwords exist before login checks (in isolated test DB).
  const seedRun = spawnSync(process.execPath, ['seed-human-users.js'], {
    cwd: process.cwd(),
    env: { ...process.env, MONGODB_URI: testDbUri },
    stdio: 'ignore',
  });
  if (seedRun.status !== 0) throw new Error('Unable to seed human users for critical flow test');
