                ...
                <div className="alerts-badges">
                  {mockAlerts.map((alert, index) => (
                    <button
                      key={alert.id}
                      type="button"
                      className={`alert-badge ${alert.type} ${selectedAlertId === alert.id ? 'active' : ''}`}
                      style={{ animationDelay: `${index * 80}ms` }}
                      onClick={() => setSelectedAlertId(alert.id === selectedAlertId ? null : alert.id)}
                      aria-expanded={selectedAlertId === alert.id}
                      aria-label={`Alerte ${alert.message}, ${alert.time}. Cliquez pour plus de détails.`}
                    >
                      <span className="badge-dot" />
                      <span className="badge-text">{alert.message}</span>
                      <span className="badge-time">{alert.time}</span>
                    </button>
                  ))}
                </div>
                {selectedAlert && (
                  <div className="alert-popover" role="region" aria-live="polite">
                    <div className={`alert-popover-header ${selectedAlert.type}`}>
                      <span>
                    ...
                    <p className="alert-popover-message">{selectedAlert.message}</p>
                    <p className="alert-popover-note">Cliquez sur une alerte pour fermer le détail.</p>
                  </div>
                )}
              </div>
