...
// Ce fichier participe a l'application mobile autour de App.
...
type Nav =
  ...
  | { screen: 'new_request'; params: { productId: string } }
  | { screen: 'my_requests' }
  | { screen: 'request_detail'; params: { id: string } }
...
export function App() {
  ...
  const content = useMemo(() => {
    ...
    if (nav.screen === 'dashboard') {
      return (
        <DashboardScreen
          ...
          onOpenRequests={() => go({ screen: 'my_requests' })}
    ...
    if (nav.screen === 'my_requests') return <MyRequestsScreen onBack={backToDashboard} onOpenDetail={(id) => go({ screen: 'request_detail', params: { id } })} />;
    if (nav.screen === 'request_detail') return <RequestDetailScreen id={nav.params.id} onBack={() => go({ screen: 'my_requests' })} />;
    ...
    if (nav.screen === 'product') {
      return (
        <ProductScreen
          ...
          onRequestProduct={(productId) => go({ screen: 'new_request', params: { productId } })}
    ...
    if (nav.screen === 'new_request') return <NewRequestScreen productId={nav.params.productId} onBack={() => go({ screen: 'product', params: { id: nav.params.productId } })} onDone={() => go({ screen: 'my_requests' })} />;
