...
// Ce fichier affiche un ecran mobile pour DashboardScreen.
...
export function DashboardScreen(props: {
  ...
  const isDemandeur = String(props.userRole || '').toLowerCase() === 'demandeur';
  ...
  useEffect(() => {
    ...
    const tryAutoSync = async () => {
      ...
      autoSyncing = true;
      try {
        const count = await OutboxRepo.countPending().catch(() => 0);
        if (disposed || count <= 0) return;
        setSyncMsg('Réseau disponible: synchronisation automatique...');
        const result = await SyncService.pushPending();
        if (!disposed) setSyncMsg(formatSyncSummary(result));
      } catch (e: any) {
        if (!disposed) setSyncMsg(e?.message || 'Erreur synchronisation');
      } finally {
        autoSyncing = false;
        if (!disposed) refresh();
      }
    ...
    const unsub = NetInfo.addEventListener((state) => {
      ...
      if (connection.canSync) void tryAutoSync();
    });
  ...
  return (
    ...
    >
      ...
      <Button title="Mes demandes" onPress={props.onOpenRequests} />
      <Button title="Catalogue produits" onPress={props.onOpenCatalog} variant="ghost" style={{ marginTop: 10 }} />
      <Button title="Mission (précharger)" onPress={props.onOpenMission} variant={isDemandeur ? 'ghost' : 'primary'} style={{ marginTop: 10 }} />
      {!isDemandeur ? (
        <>
      ...
      {!isDemandeur ? <Button title="Historique (envoyés)" onPress={props.onOpenHistory} variant="ghost" style={{ marginTop: 10 }} /> : null}
