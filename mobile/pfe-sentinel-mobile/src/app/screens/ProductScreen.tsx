...
// Ce fichier affiche un ecran mobile pour ProductScreen.
...
export function ProductScreen(props: {
  ...
  userRole?: string;
  onBack: () => void;
  onRequestProduct: (productId: string) => void;
    ...
    <Screen title="Produit" onBack={props.onBack} scroll>
      {!item ? (
        ...
        <Card>
          ...
          <View style={{ height: 10 }} />
          {isDemandeur ? (
            <Button title="Demander ce produit (offline)" onPress={() => props.onRequestProduct(item.id)} />
          ) : (
            <>
              ...
              <Button title="Demander ce produit (offline)" onPress={() => props.onRequestProduct(item.id)} variant="ghost" style={{ marginTop: 10 }} />
            </>
          )}
        </Card>
      )}
