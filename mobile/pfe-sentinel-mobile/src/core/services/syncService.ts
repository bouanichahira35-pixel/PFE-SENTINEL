...
// Ce fichier gere un service mobile lie a syncService.
  ...
  async pushPending(): Promise<SyncResult> {
    ...
    for (const e of pending) {
      try {
        ...
        const deviceMeta = payload?.meta?.device && typeof payload.meta.device === 'object'
          ? payload.meta.device
          : undefined;
        ...
        if (isRejected && !isAccepted) {
          ...
          if (classified === 'conflict') {
            await OutboxRepo.markConflict(e.id, errMsg);
            await markRequestEventRejected(e, 'conflict');
          } else {
            await OutboxRepo.markError(e.id, errMsg);
            await markRequestEventRejected(e, 'error');
          }
          failed += 1;
          continue;
        }

        await reconcileAcceptedRequestEvent(e, accepted);
        ...
        if (classified === 'conflict') {
          await OutboxRepo.markConflict(e.id, msg);
          await markRequestEventRejected(e, 'conflict');
        } else {
          await OutboxRepo.markError(e.id, msg);
          await markRequestEventRejected(e, 'error');
        }
        failed += 1;
      }
...
async function reconcileAcceptedRequestEvent(e: any, accepted: any[]) {
  if (!String(e?.type || '').startsWith('request_')) return;
  const match = accepted.find((x: any) => String(x?.id || '') === e.id);
  const result = match?.result || {};
  const remoteId = String(result?.id || '').trim();
  const status = String(result?.status || '').trim();
  if (e.type === 'request_create' && remoteId) {
    await RequestsRepo.markSyncedByEvent(e.id, remoteId, status || 'pending');
  } else {
    await RequestsRepo.markLocalStateByEvent(e.id, 'synced', status || undefined);
  }
}

async function markRequestEventRejected(e: any, state: 'error' | 'conflict') {
  if (!String(e?.type || '').startsWith('request_')) return;
  await RequestsRepo.markLocalStateByEvent(e.id, state);
}
